...
export default {
  ...
  methods: {
    async getMovies(){
      const URL = `${BASE_URL}movie/now_playing?api_key=${APIKEY}&language=en-US&page=1`;
    ...
    async searchMovies(){
      const URL = `${BASE_URL}search/movie?api_key=${APIKEY}&language=en-US&page=1&query=${this.searchInput}`;
      const data = axios.get(URL);
      const result = await data;
      result.data.results.forEach(movie => {
